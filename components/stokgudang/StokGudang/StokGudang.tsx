...
const StokGudang: React.FC = () => {
    ...
    useEffect(() => {
        fetchStoks();
    }, [currentPage, selectedKategori, startDate, endDate, sortConfig]);
    return (
        <div className="p-6">
            ...
            <div className="bg-white p-4 rounded-lg shadow mb-6">
                ...
                <span className={`px-3 py-1 inline-flex text-xs leading-5 font-bold rounded-full uppercase tracking-wider
                    ${stok.status === 'aman' ? 'bg-green-100 text-green-800' :
                    stok.status === 'rusak' ? 'bg-red-100 text-red-800' :
                        stok.status === 'cacat' ? 'bg-yellow-100 text-yellow-800' :
