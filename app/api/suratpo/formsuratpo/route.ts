...
export async function POST(req: Request) {
    try {
        ...
        if (!po_number || !date || !supplier || !address || !shipping_address || !items.length) {
        ...
        const [result]: any = await db.query(
            `INSERT INTO purchase_orders (po_number, date, supplier, address, attention, note, shipping_address, total_amount, subtotal, tax, grand_total)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [po_number, date, supplier, address, attention, note, shipping_address, subtotal, subtotal, tax, grand_total]
        );
