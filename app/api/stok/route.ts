...
function formatDateForMySQL(dateString: string | Date): string {
...
export async function GET(request: Request) {
  try {
    ...
    const formattedRows = rows.map((row) => ({
      ...
      stok_keluar: Number(row.stok_keluar ?? 0),
      stok_sisa: Number(row.stok_sisa ?? 0),
      satuan: row.satuan,
      lokasi: row.lokasi,
  ...
  try {
    ...
    if (invalidFields.length > 0) {
      return NextResponse.json(
        {
          success: false,
          message: `Invalid or missing fields: ${invalidFields
            .map(([key]) => key)
            .join(", ")}`,
    ...
    try {
      ...
      if (existingStok.length > 0) {
        ...
        const updatedStokMasuk =
          Number(existingData.stok_masuk ?? 0) + validations.stok_masuk;
        const updatedStokSisa =
          Number(existingData.stok_sisa ?? 0) + validations.stok_masuk;
        ...
        // Gunakan formatDateForMySQL untuk mengubah format tanggal
        const [result] = await db.query<ResultSetHeader>(
          ...
          [
            ...
            validations.satuan,
            validations.lokasi,
            formatDateForMySQL(validations.tanggal_entry), // Format tanggal untuk MySQL
            validations.tanggal_masuk ? formatDateForMySQL(validations.tanggal_masuk) : null, // Format tanggal untuk MySQL
    ...
    return NextResponse.json(
      {
        success: false,
        message: "Gagal memproses data stok",
        error: error instanceof Error ? error.message : "Unknown error",
      },
      { status: 500 }
    );
